...
class KeywordsGeneratorService:
    ...
    async def generate_keywords(self) -> list[Keyword]:
        print_c("warning",f"génération encours sur le : {self.document.name}")
        ...
        print_c("success", f"calcul du tf déjà fait ! {len(terms_with_tf_score)} - {len(self.document.all_terms)}")
        ...
        print_c("success", f"calcul du idf, vient d'être fait aussi ! {len(terms_with_idf_score)} - {len(self.document.all_terms)}")
        ...
        print_c("success", f"calcul du tf_idf enfin fait aussi ! {len(terms_with_tf_idf_score)} - {len(self.document.all_terms)}")

        self.document.__setitem__("all_unique_terms",terms_with_tf_idf_score)
        print_c("warning", "les mots-clés sont presque prêt à être généré")
